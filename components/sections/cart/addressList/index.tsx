import { CardAddress } from "components/sections/profile/my-addresses/list-addresses/card-address";
import { ListAddresses } from "components/sections/profile/my-addresses/list-addresses/styles";
import { useProfile } from "src/contexts/Profile";
import * as S from "./styles";
import { Address } from "src/services/profile";
import { SetStateAction } from "react";

interface AddressListProps {
  closeAddressList: React.Dispatch<boolean>;
  addressSelected: Address | null;
  setAddressSelected: React.Dispatch<SetStateAction<Address | null>>;
}

export function AddressList({
  addressSelected,
  closeAddressList,
  setAddressSelected
}: AddressListProps) {
  const { address } = useProfile();

  return (
    <S.AddressList>
      <ListAddresses>
        <div className="top-list-address">
          <button
            type="button"
            onClick={() => closeAddressList(false)}
            aria-label="close creation address"
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path d="M0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256zM175 208.1L222.1 255.1L175 303C165.7 312.4 165.7 327.6 175 336.1C184.4 346.3 199.6 346.3 208.1 336.1L255.1 289.9L303 336.1C312.4 346.3 327.6 346.3 336.1 336.1C346.3 327.6 346.3 312.4 336.1 303L289.9 255.1L336.1 208.1C346.3 199.6 346.3 184.4 336.1 175C327.6 165.7 312.4 165.7 303 175L255.1 222.1L208.1 175C199.6 165.7 184.4 165.7 175 175C165.7 184.4 165.7 199.6 175 208.1V208.1z" />
            </svg>
          </button>
        </div>

        <div>
          {address?.map((item) => {
            if (item?.id === addressSelected?.id) {
              return <div></div>;
            }

            return (
              <CardAddress key={"address" + item.id} addressInformations={item} addressSelected={addressSelected} setAddressSelected={setAddressSelected} closeAddressList={closeAddressList}/>
            );
          })}
        </div>
      </ListAddresses>
    </S.AddressList>
  );
}
