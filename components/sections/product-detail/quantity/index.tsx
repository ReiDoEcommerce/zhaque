...
export function Quantity({ product }: QuantityProps) {
    ...
    const { state, cartLoading, AddProductInCart, PlusCart } = useCart()
    ...
    async function handleAddToCart() {
        ...
        await AddProductInCart(thisProduct, quantity);
    }
        ...
        <>
            <S.Quantity>
                ...
                <div className="numbers-list">
                    {[1, 2, 5, 10, 25, 50].map((item) => {
                        return <button key={"qtd" + item} type="button" className="paragraph-1-bold" onClick={() => setQuantity((old) => old + item)}>
                            + {item}
                        </button>
                    })}
                </div>
                ...
                <div className="quantity-total">
                    <h5 className="paragraph-1-bold">
                        Quantidade total:
                    
                        <button type="button" onClick={() => setQuantity(0)}>limpar</button>
                    </h5>
                    ...
                    <div className="quantity">
                        ...
                        <span>
                            <span className="title-2-bold">{quantity}</span>
                        </span>
            ...
            <div className="actions">
                <button type="button" className="add-to-cart paragraph-1-bold" onClick={handleAddToCart} disabled={cartLoading}>
                    <span>
                    ...
                    <span>{ToMoney(quantity * Number(product.preco))}</span>
                </button>
            </div>
        </>
    )
